...
class Playlist:
    ...
    def add_playlist(self, user_account):
        ...
        user_selection = input("\nEnter the number(s) of the playlists to be added (e.g. 0,1,3) or ALL: ")
        if user_selection.lower() == "all":
            ...
            try:
                ...
                print("Invalid selection. Playlists must be selected using the number displayed to the left of their name.\n")
                return
    ...
    def filter_tracks_by_key(self, filter, user_account):
        ...
        for track in self._tracks:
            spotify_key = (
                user_account._auth_sp.audio_features([track])[0]['key'],
                user_account._auth_sp.audio_features([track])[0]['mode']
                )
            camelot_key = self.convert_key(spotify_key)
    ...
    def sort_tracks_by_tempo(self, user_account, ascending_order):
        ...
        if len(self._tracks) > 0:
            ...
            print("Your playlist is empty. Add to the playlist before sorting.\n")
